...
class DatasetLu2018Bvoc():
  ...
  def __init__(self, fname):
    ...
    y: all the land grid, loop for 10 years
    """
    
    # Use short names
    nyear = DatasetLu2018Bvoc.nyear
    ...
    if not DatasetLu2018Bvoc.coord_is_set:
      DatasetLu2018Bvoc.set_coordination(lon_raw[0,:], lat_raw[0,:])

    # BVOC emissions
    self.emis = np.transpose( np.reshape( raw[:, 3:15], (nyear, ngrid, nmon) ), (0, 2, 1) )  # [nyear, nmon, ngrid]

    # Average over years for emission
    self.emis_avg = np.nanmean(self.emis, axis=0)  # [nmon, ngrid]
  
  
  @classmethod
  def set_coordination(cls, lon_land, lat_land):
    # number of grids on the land
    cls.ngrid = lon_land.size
  
    # Lon_land and lat_land keep the same for each year
    cls.lon_land = lon_land
    cls.lat_land = lat_land
  
    # Global reduced Gaussian grid, west --> east, south --> north
    cls.lon_rg, cls.lat_rg = generate_global_reduced_gaussian_grid(nlon_N80, lat_N80)
      
    # Global regular 1x1 grid, west --> east, south --> north
    cls.lon_11 = np.linspace(-179.5, 179.5, 360)
    cls.lat_11 = np.linspace(-89.5, 89.5, 180)
      
    # Global Gaussian grid with 360 regular lon, west --> east, south --> north
    cls.lon_gg = np.copy(DatasetLu2018Bvoc.lon_11)
    cls.lat_gg = np.copy(DatasetLu2018Bvoc.lat_rg)
    
    # Land ind in global grid for RG grid
    cls.land_ind = get_land_ind_in_glob_reduced_gaussian_grid( \
      lon_land, lat_land, cls.lon_rg, cls.lat_rg)
    
    # Coordination is set
    cls.coord_is_set = True
  ...
  def get_dominant_vegetation_type(vt):
    ...
    if DatasetLu2018Bvoc.ngrid is None:
      print('Warning: Initiate an instance first to set ngrid.')
      return None
    else:
      ngrid = DatasetLu2018Bvoc.ngrid
  ...
  def get_data_on_regular_grid(self):
    # Use short names
    lon_gg, lat_gg = DatasetLu2018Bvoc.lon_gg, DatasetLu2018Bvoc.lat_gg
    nlon_gg, nlat_gg = lon_gg.size, lat_gg.size
    
    lon_11, lat_11 = DatasetLu2018Bvoc.lon_11, DatasetLu2018Bvoc.lat_11
    nlon_11, nlat_11 = lon_11.size, lat_11.size
    
    # Initialize the data array
    self.emis_avg_gg = np.zeros( (nmon, nlat_gg, nlon_gg) )
    self.emis_avg_11 = np.zeros( (nmon, nlat_11, nlon_11) )

    # Interpolate cvl_dom_avg and cvh_dom_avg for each month with linear method
    for im in range(nmon):
      print('{0:d}'.format(int(im+1)))
      self.emis_avg_gg[int(im), :, :], self.emis_avg_11[int(im), :, :] = \
        DatasetLu2018Bvoc.interpolate_data_from_lrg_to_11(self.emis_avg[int(im), :], kind='linear')
  ...
  def save_data(self, fname):
    # Save the data
    np.savez(fname, \
             lon_land = DatasetLu2018Bvoc.lon_land, \
             lat_land = DatasetLu2018Bvoc.lat_land, \
             lon_11 = DatasetLu2018Bvoc.lon_11, \
             lat_11 = DatasetLu2018Bvoc.lat_11, \
             emis_avg = self.emis_avg, \
             emis_avg_11 = self.emis_avg_11, \
            )
